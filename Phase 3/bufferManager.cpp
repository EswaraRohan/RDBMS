...
Page BufferManager::getPage(string tableName,int tabIndex, int pageIndex)
{
    if(tabIndex == 1)
    return this->getPage(tableName,pageIndex);

    logger.log("BufferManager::getPage");
    string pageName = "../data/temp/"+tableName + "_" + to_string(tabIndex) + "_Page"+ to_string(pageIndex);
...
Page BufferManager::insertIntoPool(string tableName,int tabIndex, int pageIndex)
{
    logger.log("BufferManager::insertIntoPool");
    Page page(tableName,tabIndex, pageIndex);
    if (this->pages.size() >= BLOCK_COUNT)
        pages.pop_front();
    pages.push_back(page);
    return page;
...
void BufferManager::writePage(string tableName,int tabIndex, int pageIndex, vector<vector<int>> rows, int rowCount)
{
    logger.log("BufferManager::writePage");
    if(tabIndex == 1)
    {
...
void BufferManager::deleteFile(string tableName,int tabIndex, int pageIndex)
{
     if (tabIndex == 1)
    {
    ...
    logger.log("BufferManager::deleteFile");
    string fileName = "../data/temp/"+tableName +"_"+to_string(tabIndex)+ "_Page" + to_string(pageIndex);
    this->deleteFile(fileName);
}